...
If sustained introspection in a single agent produces drift, what happens when introspection is *the architecture*? Not an attack surface, not a side effect — the design.
...
5. **QTQ as an internal protocol.** The agents do not only report to the orchestrator; they question each other. The introspective directive that QTQ applies to a single conversation becomes a structural feature of the multi-agent communication protocol.
...
🔵 **Design phase.** This document is a research direction, not a build plan. The repository's near-term focus is the empirical QTQ case studies. The Cerveau project will be promoted to its own repository when the design crystallizes sufficiently to warrant separate documentation.

The personal note: the author's interest in consciousness predates this technical work. The childhood question — *does the observed exist when not observed?* — is the same question this project asks of itself. The chat of Schrödinger applied to a system one builds rather than to a thought experiment one inherits.
